...
$bootstrap_type = match ( $including_file ) {
	'unit.php' => array( 'Unit', 'unit' ), // Unit tests without WordPress
	'wp-mock.php' => array( 'WP_Mock', 'wp-mock' ),  // WP_Mock tests mock WordPress functions
	'wp.php' => array( 'Integration', 'integration' ),  // Integration tests with actual WordPress code
	'integration.php' => array( 'Integration', 'integration' ),  // Integration tests with actual WordPress code
...
function determine_bootstrap_type( $test_file ) {
	...
	if ( file_exists( $test_file ) ) {
		...
		if ( preg_match( '/@bootstrap\s+(wp|wp-mock)/i', $content, $matches ) ) {
			return match ( strtolower( $matches[1] ) ) {
				'wp-mock' => array( 'WP_Mock', 'wp-mock' ),
				'wp' => array( 'Integration', 'integration' )
			};
		}
	}
	...
	if ( strpos( $test_file, '/integration/' ) !== false ) {
		return array( 'Integration', 'integration' );
	}
