...
class Test_FileHandler extends Unit_Test_Case {
	...
	public function invalidPathProvider(): array {
		return array(
			'empty path'               => array( '' ),
			'invalid characters'       => array( 'path/with/invalid/*/chars' ),
			'absolute path with drive' => array( 'C:\\path\\to\\file' ),
			'too long path'            => array( str_repeat( '/a', 4096 ) ),
			'non-string path'          => array( 42 ),
		);
	}
	...
	public function invalidContentProvider(): array {
		return array(
			...
			'resource content' => array( fopen( 'php://memory', 'r' ) ),
			'callable content' => array( function () {} ),
		);
	}
}
