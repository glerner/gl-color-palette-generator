...
class Test_Settings_Manager extends Unit_Test_Case {
	...
	public function invalidSettingKeyProvider(): array {
		return array(
			'empty key'          => array( '' ),
			'invalid characters' => array( 'invalid@key' ),
			'too long'           => array( str_repeat( 'a', 256 ) ),
			'reserved prefix'    => array( 'wp_test_setting' ),
		);
	}
	...
	public function invalidSettingValueProvider(): array {
		return array(
			'invalid object'   => array( new \stdClass() ),
			'invalid resource' => array( fopen( 'php://memory', 'r' ) ),
			'invalid callable' => array( function () {} ),
			'invalid binary'   => array( pack( 'H*', 'deadbeef' ) ),
		);
	}
}
