...
class Test_HuggingFace_Provider extends Test_Provider_Mock {
	...
	public function test_validate_credentials() {
		$provider = new HuggingFace_Provider( new Provider_Config( array() ) );
		$this->assertInstanceOf( \WP_Error::class, $provider->validate_credentials() );

		$provider = new HuggingFace_Provider( new Provider_Config( array( 'api_key' => 'test_key' ) ) );
		$this->assertInstanceOf( \WP_Error::class, $provider->validate_credentials() );
	...
	public function test_generate_palette() {
		...
		WP_Mock::userFunction( 'wp_remote_post' )->once()->andReturn(
			array(
				...
				'body'     => json_encode(
					array(
						'generated_text' => json_encode( array( 'colors' => array( '#FF0000', '#00FF00', '#0000FF' ) ) ),
					)
				),
			)
		);
