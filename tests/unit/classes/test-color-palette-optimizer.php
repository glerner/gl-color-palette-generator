...
class Test_Color_Palette_Optimizer extends Unit_Test_Case {
	...
	public function test_invalid_strategy(): void {
		...
		$this->optimizer->optimizePalette( $this->test_palette, array( 'strategy' => 'invalid' ) );
	}
	...
	public function test_optimize_for_balance(): void {
		$optimized = $this->optimizer->optimizePalette(
			$this->test_palette,
			array(
				'strategy'    => 'balance',
				'hue_spacing' => 45,
	...
	public function it_improves_contrast_ratios( $color1, $color2, $expected_improvement ) {
		$palette = new Color_Palette( array( $color1, $color2 ) );
	...
	public function contrastPairsProvider() {
		return array(
			'similar colors'        => array( '#666666', '#777777', true ),
			'low contrast'          => array( '#FFFF00', '#FFFFFF', true ),
			'already good contrast' => array( '#000000', '#FFFFFF', false ),
		);
	}
	...
	public function it_optimizes_for_different_wcag_levels() {
		...
		$aa_optimized = $this->optimizer->optimize(
			$palette,
			array(
				'target_wcag' => 'AA',
			)
		...
		$aaa_optimized = $this->optimizer->optimize(
			$palette,
			array(
				'target_wcag' => 'AAA',
			)
	...
	private function get_saturation_variance( array $colors ): float {
		$saturations = array_map( array( $this, 'get_saturation' ), $colors );

		$mean             = array_sum( $saturations ) / count( $saturations );
		$squared_diff_sum = array_sum(
			array_map(
				function ( $s ) use ( $mean ) {
					return pow( $s - $mean, 2 );
				},
