...
class Test_Palette_Cache extends Unit_Test_Case {
	...
	public function setUp(): void {
		...
		$this->sample_params = array(
			'base_color' => '#FF0000',
			'scheme'     => 'complementary',
			'variation'  => 'light',
		);

		$this->sample_palette = array(
			array( '#FF0000', 'Red' ),
			array( '#00FF00', 'Green' ),
			array( '#0000FF', 'Blue' ),
		);
	...
	public function test_clear_all() {
		...
		$this->cache->set( $this->sample_params, $this->sample_palette );
		$this->cache->set( array( 'different' => 'params' ), array( 'different' => 'palette' ) );
		...
		$this->assertFalse( $this->cache->get( array( 'different' => 'params' ) ) );
	}
