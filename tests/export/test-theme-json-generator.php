...
class Test_Theme_JSON_Generator extends WP_UnitTestCase implements Color_Constants {
    ...
    public function test_invalid_scheme_type(): void {
        ...
        $this->instance->generate_theme_json(['primary' => '#000000'], 'invalid_scheme');
    }
    ...
    public function test_missing_required_colors(): void {
        ...
        $this->instance->generate_theme_json(['secondary' => '#000000'], 'complementary');
    }
    ...
    public function test_generate_style_variations(): void {
        ...
        $this->assertCount(self::VARIATIONS_PER_MODE * 2, $variations,
        ...
        foreach ($variations as $variation) {
            ...
            if ($base_luminance > self::LIGHT_LUMINANCE_THRESHOLD) {
                $light_variations++;
            } else if ($base_luminance < self::DARK_LUMINANCE_THRESHOLD) {
                $dark_variations++;
            } else {
            ...
            $found_colors = ['primary' => false, 'secondary' => false, 'tertiary' => false, 'accent' => false];
            foreach ($palette as $color) {
        ...
        $this->assertEquals(self::VARIATIONS_PER_MODE, $light_variations,
            'Should have correct number of light variations');
        $this->assertEquals(self::VARIATIONS_PER_MODE, $dark_variations,
    ...
    public function test_invalid_style_variation_colors(): void {
        ...
        $this->instance->generate_style_variations(['#FF0000']);

        // Test with invalid color format
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Invalid color format');
        $this->instance->generate_style_variations(['not-a-color', '#00FF00']);

        // Test with empty array
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('No colors provided');
        $this->instance->generate_style_variations([]);
    }
    ...
    public function test_invalid_theme_json_colors(): void {
        ...
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Invalid color format');
        $this->instance->generate_theme_json(['primary' => 'not-a-color'], 'complementary');

        // Test with empty array
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('No colors provided');
        $this->instance->generate_theme_json([], 'complementary');
    }
    ...
    public function test_save_style_variations() {
        $base_colors = [self::COLOR_NEAR_BLACK, self::COLOR_OFF_WHITE];
        $variations = $this->instance->generate_style_variations($base_colors);
        ...
        foreach ($variations as $variation) {
            ...
            foreach ($palette as $color) {
                switch ($color['slug']) {
                    ...
                    case 'base-2':
                        if ($is_light_mode) {
                            $this->assertEquals(self::COLOR_WHITE, $color['color'],
                                'base-2 should be white in light mode');
                        } else {
                            $this->assertEquals(self::COLOR_OFF_WHITE, $color['color'],
                    ...
                    case 'contrast-3':
                        ...
                        if ($base_luminance > self::LIGHT_LUMINANCE_THRESHOLD) {
                            ...
                            $this->assertLessThan(self::DARK_LUMINANCE_THRESHOLD, $luminance,
                                "$color[slug] should be very dark in light mode");
                        } else {
                            // Dark mode: contrast should be very light
                            $this->assertGreaterThan(self::LIGHT_LUMINANCE_THRESHOLD, $luminance,
    ...
    public function test_create_variations_light_mode() {
        ...
        $variations = $generator->generate_tints_and_shades(self::COLOR_NEAR_BLACK, [
            'is_dark_mode' => false
        ]);
    ...
    public function test_create_variations_dark_mode() {
        $generator = new Color_Variation_Generator($this->instance->get_contrast_checker());
        $variations = $generator->generate_tints_and_shades(self::COLOR_WHITE, [
            'is_dark_mode' => true
        ]);
    ...
    public function test_accessible_shades() {
        $generator = new Color_Variation_Generator($this->instance->get_contrast_checker());
        $variations = $generator->generate_tints_and_shades(self::COLOR_MID_GRAY, [
            'is_dark_mode' => false
        ]);
    ...
    public function test_theme_json_color_slug_compatibility(): void {
        ...
        $required_slugs = ['primary', 'secondary', 'tertiary', 'accent'];

        foreach ($required_slugs as $slug) {
            $this->assertArrayHasKey($slug, array_column($palette, 'color', 'slug'),
            ...
            foreach ($palette as $color) {
                if ($color['slug'] === $slug) {
                    ...
                    switch ($slug) {
                        ...
                        case 'contrast':
                            $this->assertEquals(self::COLOR_NEAR_BLACK, $color['color'],
                        ...
                        case 'accent':
                            $this->assertEquals($base_colors[3], $color['color'],
    ...
    public function test_theme_compatibility(): void {
        ...
        $theme_slugs = [
            ...
            'primary', 'primary-lighter', 'primary-light', 'primary-dark', 'primary-darker',
            'secondary', 'secondary-lighter', 'secondary-light', 'secondary-dark', 'secondary-darker',
            'tertiary', 'tertiary-lighter', 'tertiary-light', 'tertiary-dark', 'tertiary-darker',
            'accent', 'accent-lighter', 'accent-light', 'accent-dark', 'accent-darker',

            // TwentyTwentyFour/Five style
            'base', 'base-2',
            'contrast', 'contrast-2', 'contrast-3',
        ...
        foreach ($variations as $variation) {
            ...
            foreach (['primary', 'secondary', 'tertiary', 'accent'] as $role) {
                if (isset($color_groups[$role])) {
                    ...
                    usort($role_colors, function($a, $b) {
        ...
        foreach ($tt3_slugs as $slug) {
            ...
            foreach ($palette as $color) {
                if ($color['slug'] === $slug) {
                    ...
                    switch ($slug) {
                        case 'primary':
                            $this->assertEquals($base_colors[0], $color['color'],
                                'Primary color should match input color');
                            break;

                        case 'secondary':
                            $this->assertEquals($base_colors[1], $color['color'],
                                'Secondary color should match input color');
                            break;

                        case 'tertiary':
                            $this->assertEquals($base_colors[2], $color['color'],
    ...
    public function test_contrast_ratios() {
        ...
        foreach ($test_colors as $base_color) {
            $variations = $generator->generate_tints_and_shades($base_color, [
                'is_dark_mode' => false
            ]);
