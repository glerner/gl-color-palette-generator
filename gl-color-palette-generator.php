...
function gl_cpg_check_compatibility() {
	...
	if ( version_compare( $php_version, $min_php_version, '<' ) ) {
		$compatible = false;
		$messages[] = sprintf(
			__( 'GL Color Palette Generator requires PHP version %1$s or higher. Your current version is %2$s.', 'gl-color-palette-generator' ),
			$min_php_version,
			$php_version
		);
	}
	...
	if ( version_compare( $wp_version, $min_wp_version, '<' ) ) {
		$compatible = false;
		$messages[] = sprintf(
			__( 'GL Color Palette Generator requires WordPress version %1$s or higher. Your current version is %2$s.', 'gl-color-palette-generator' ),
			$min_wp_version,
			$wp_version
		);
	}
