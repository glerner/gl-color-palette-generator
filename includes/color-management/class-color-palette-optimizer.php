...
class Color_Palette_Optimizer {
     ...
     */
    public function optimize(Color_Palette $palette, array $options = []): Color_Palette {
        $default_options = [
            'target_wcag' => 'AA',
            'preserve_hues' => true,
            'max_adjustment' => 0.2
    ...
    private function normalize_saturation(array $colors, float $max_adjustment): array {
        $saturations = array_map([$this->analyzer, 'get_saturation'], $colors);
        $mean_saturation = array_sum($saturations) / count($saturations);

        return array_map(function($color) use ($mean_saturation, $max_adjustment) {
    ...
    private function balance_values(array $colors, float $max_adjustment): array {
        ...
        return array_map(function($color, $index) use ($mean_value, $max_adjustment) {
            $hsv = $this->hex_to_hsv($color);
            $current_value = $hsv['v'];
    ...
    private function distribute_hues(array $colors, float $max_adjustment): array {
        ...
        return array_map(function($index) use ($colors, $target_step, $max_adjustment) {
