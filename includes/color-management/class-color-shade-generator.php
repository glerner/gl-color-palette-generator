...
class Color_Variation_Generator implements Color_Shade_Generator_Interface {
	 ...
	 */
	public function generate_tints_and_shades( string $color, array $options = array() ): array {
		...
		$variations = array( 'base' => $color ); // Always store base color
	 ...
	 */
	public function generate_contrast_color( string $background_color, bool $is_dark_mode, array $options = array() ): string {
	...
	public function generate_contrasting_variation( string $base_color, bool $prefer_lighter = true ): string {
		...
		while ( $attempts < Color_Constants::COLOR_METRICS['lightness']['max_attempts'] ) {
			...
			$contrast_hsl = $this->adjust_lightness(
				$contrast_hsl,
				$prefer_lighter ?
				Color_Constants::COLOR_METRICS['lightness']['step'] :
				-Color_Constants::COLOR_METRICS['lightness']['step']
	 ...
	 */
	public function analyze_color( string $color, array $options = array() ): array {
		$rgb = $this->color_utility->hex_to_rgb( $color );
		$hsl = $this->color_utility->rgb_to_hsl( $rgb );
