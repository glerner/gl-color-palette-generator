...
class Core implements Plugin_Lifecycle, Component_Registry, WordPress_Integration {
    ...
    public function init(): bool {
        try {
            ...
            add_action('admin_notices', [$this, 'show_initialization_error']);
            return false;
        }
    }
    ...
    public function register_hooks(): void {
        // Admin hooks
        add_action('admin_menu', [$this->get_component('admin_interface'), 'register_menus']);
        add_action('admin_notices', [$this->get_component('admin_notices'), 'display_notices']);
        add_action('admin_enqueue_scripts', [$this->get_component('admin_interface'), 'enqueue_assets']);

        // AJAX handlers
        add_action('wp_ajax_gl_generate_palette', [$this->get_component('palette_generator'), 'ajax_generate']);
        add_action('wp_ajax_gl_analyze_palette', [$this->get_component('color_metrics'), 'analyze_palette']);
        add_action('wp_ajax_gl_export_palette', [$this->get_component('color_exporter'), 'ajax_export']);

        // REST API
        add_action('rest_api_init', [$this, 'register_rest_routes']);

        // Cron jobs
        add_action('gl_daily_cleanup', [$this->get_component('cache_manager'), 'cleanup']);
    }
    ...
    public function register_rest_routes(): void {
        foreach (['palette_generator', 'color_metrics', 'color_exporter'] as $component) {
