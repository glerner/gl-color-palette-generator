...
class Palm_Provider extends AI_Provider_Base {
	...
	public function validate_credentials(): bool|WP_Error {
		...
		$response = wp_remote_post(
			'https://generativelanguage.googleapis.com/v1beta/models/' . $this->model . ':generateText',
			array(
				'headers' => array(
	...
	public function generate_palette( array $params ): array|WP_Error {
		...
		$response = wp_remote_post(
			'https://generativelanguage.googleapis.com/v1beta3/models/' . $this->model . ':generateText',
			array(
				'headers' => array(
					'Authorization' => "Bearer {$this->api_key}",
					'Content-Type'  => 'application/json',
