...
class Name_Generator {
	...
	public function generate_name( $colors, $options = array() ) {
		$theme = $options['theme'] ?? '';
		$mood  = $options['mood'] ?? '';
	...
	public function get_name_suggestions( $colors ) {
		...
		$suggestions = array();
		$themes      = array( 'nature', 'ocean', 'sunset', 'urban', 'forest' );
		$moods       = array( 'calm', 'energetic', 'professional', 'playful', 'elegant' );

		foreach ( $themes as $theme ) {
			$suggestions[] = $this->generate_name( $colors, array( 'theme' => $theme ) );
		}

		foreach ( $moods as $mood ) {
			$suggestions[] = $this->generate_name( $colors, array( 'mood' => $mood ) );
		}
