...
class VisualizationHelper {
	 ...
	 * Preview types
	 */
	private const PREVIEW_TYPES = array(
		'text'       => array( 'paragraph', 'heading', 'button' ),
		'ui'         => array( 'card', 'alert', 'form' ),
		'pattern'    => array( 'dots', 'stripes', 'grid' ),
		'simulation' => array( 'normal', 'protanopia', 'deuteranopia', 'tritanopia' ),
		'contrast'   => array( 'side-by-side', 'overlay', 'gradient' ),
	);
	 ...
	 */
	public function generate_previews( $foreground, $background, $options = array() ) {
		$previews = array(
	...
	private function generate_text_sample( $foreground, $background, $tag, $text, $styles ) {
		$style_string = $this->css_generator->generate_inline_styles(
			array_merge(
				$styles,
				array(
					'color'            => $foreground,
