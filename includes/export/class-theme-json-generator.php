...
class Theme_Json_Generator implements Color_Constants {
    ...
    public function generate_theme_json_variations($colors, $scheme = 'complementary') {
        ...
        foreach ($required_roles as $role) {
            if (!isset($colors[$role]) && !in_array($role, ['base', 'contrast', 'neutral'])) {
                return new \WP_Error(
                    'missing_color_role',
                    sprintf(__('Missing required color role: %s for scheme: %s', 'gl-color-palette-generator'), $role, $scheme)
                );
            }
        }
        ...
        foreach ($colors as $role => $color) {
            $light_variations[$role] = $this->generate_accessible_shades($color, [
        ...
        foreach ($colors as $role => $color) {
            $dark_variations[$role] = $this->generate_accessible_shades($color, [
                'contrast_level' => self::WCAG_CONTRAST_MIN,
                'small_text' => true,
                'include_base' => true
            ]);
